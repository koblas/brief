...
- Frontmatter line offset computed twice (`scaffold.FinishFS`, `assemble` check's `parsedStep`);
  `stepfile.ParseFrontmatter` should return it beside `rest`. Re-gate MINOR; deferred — unowned.
