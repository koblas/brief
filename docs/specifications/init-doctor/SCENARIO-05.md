...
- [x] Step 14: mutation-verified individually, each copied to `$TMPDIR` first and restored after, `diff` confirming byte-identical restore every time:
  - (a) dropped `featureContainingLexical`'s `rel == "."`/`".."`/prefix guard (the whole "outside-root" clause) → at the `internal/assemble` level this reddened three tests (`ReturnsFalseForAPathOutsideTheRoot`, `ReturnsFalseForADotDotEscape`, and `ReturnsTheFeatureNameThroughASymlinkedAncestor`, which now returns `".."` instead of `"auth"`) since the plan's guard bundles three related checks in one `if`; at the `internal/cli` level — the granularity the plan's claim describes — exactly one test reddened, `Test_check_hook_is_silent_when_the_edited_path_is_outside_the_feature_root`. Confirmed as specified.
  ...
  - (e) returned `errCheckFindings` instead of `nil` after `WriteHookContext` → reddened five tests, not one: every `check_hook_test.go` test that reaches the ERROR-finding branch and asserts `require.NoError(t, err)` reddened (the Step 8 test plus both control-arm tests from Step 9's three pairs that land on the ERROR side: inside-the-root, same-tree-has-.brief.yaml, and same-feature-has-open-step). This is broader than the plan's "only the Step 8 exit-0 assertion" claim because every control arm this developer wrote also asserts `require.NoError` before checking stdout — a stronger, not weaker, guard than the plan anticipated. No test outside this branch reddened.
