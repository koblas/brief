...
- [x] Step 15: mutation verification, one at a time, stashed. Required list is in Handoff *Traps*; report which test each one reddens
  ...
  - (g), added during review: `usageFix`'s `idx != -1 && strings.HasSuffix(msg, "'")` guard weakened to `idx != -1` alone — reddened exactly the two subtests of `Test_json_mode_usage_error_fix_stops_at_the_quote_when_the_line_has_trailing_prose` (`empty name`, `whitespace name`) and nothing else in the package: every other message in the suite either ends in `'` (both forms agree) or carries no `"; run '"` marker at all (both fall back the same way).
