...
- `finishLong`'s `jsonFieldsParagraph(...)` call now lists `dropped_entries` after `modified`, discovered necessary during Verify: `Test_every_command_help_names_its_json_documents_top_level_fields` decodes the live `--json` document and requires every non-header key to be a whole word in that command's own JSON paragraph, so leaving the call untouched left the full suite red. `Test_help_finish_json_is_the_exact_document` and `Test_prints_finish_flag_prose_in_its_flag_table` were updated to match (see Step 10 above). This plan's own original assumption that no test pinned this field list was wrong.

**Left unbuilt** — named so nobody assumes it exists:
- `finishLong`'s drop-reporting **prose** paragraph, before the JSON paragraph — SCENARIO-10. The JSON field-key list itself is already done (see above).
