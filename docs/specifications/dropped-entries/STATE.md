...
(overlapping configured headings, an ignored stdout write error), then corrected its own
fixes: a nested-subsection mis-attribution and a silently-unprintable error.
...
- The diff is **pooled**: old entries across all four state headings and new entries across
  all four are matched as one multiset on normalized text, sorted by **old-file line**.
  Surplus old occurrences of a duplicated text — the tail of `idxs` in old-file line order —
  are dropped (`idxs[len(idxs)-surplus:]`). `poolOccurrences` first keeps each physical line
  once: nested configured headings, or one with no leading `#` (section never terminates),
  let more than one heading's own scan reach the same line. That line goes to whichever
  heading whose scan *actually reached it* has the greatest `markdown.HeadingLine` — never
  "nearest by line number" (a section can already have closed, past an intervening
  unconfigured heading) and never "last scanned" (config order need not match nesting order).
  Getting either wrong once false-dropped a move and once mis-attributed an unconfigured
  subsection's drop.
- `entryItemText` strips only `- `/`* `/`N. `, never a `[ ]`/`[x]` checkbox, so
  ...
  thematic break (`* * *`, `- - -`, `***`, `---`) reads like a bullet marker but is checked
  first (`thematicBreakRe`) and is never an entry.
...
- `DroppedEntry.Line` is whole-body 1-based over the **old** body only. `.Heading` is display
  ...
  `assemble.Finding`/`doctor.Check`'s shape — `cli` only stringifies it. Drops are computed
  only on `FinishFS`'s `refinishWrite` path, before `applyFinishWrites`; `refinishNoop`
  returns its own literal `Dropped: []DroppedEntry{}`. D5 holds at two points: the refinish
  switch, and `runFinish`'s error branch never rendering `res` on a non-nil `err`.
- `cli`'s row detail is built by one helper (`dropDetail`/`dropExcerpt`, cut at 80 runes) and
  ...
  `fmt.Errorf` (`main.go` prints nothing itself; only `reporter.refusal` writes a visible
  line). `finishDocument.DroppedEntries` is the last field, never `omitempty`; a new `--json`
  field must update `jsonFieldsParagraph` too.
- `droppedEntries` scans only `scannableHeadings(headings)`, computed once per call: an empty
  ...
  drop-reporting paragraph pinned byte-identical in two goldens (`help_test.go`,
  `help_json_test.go`).
...
- `markdown.HeadingLine`, `Section` and `Entries` match a configured heading by **literal
  ...
  `status` and `start` all return one bare today, a wider pre-existing bug this fix avoids
  but does not close.
