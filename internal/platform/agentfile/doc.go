...
// another plugin's "<plugin>:<name>", or a bare name — against the
// filesystem into a Binding, and (Binding).LackingSkill is the single
// decision point behind "does this binding's own agent preload a given
// skill", shared by doctor's roles/roles-skill rows and setup's own
// missing-skill report. Find, Load, Parse and ResolveBinding only locate,
// decode and classify; none writes or edits an agent file — that is a
// caller's own concern: doctor reports what it finds, and setup's own
