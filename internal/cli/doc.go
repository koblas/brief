...
// [--force]" installs internal/setup's own config file and feature root,
// converging on a second run; its refusal — a *setup.RefusalError, checked
// ahead of a bare *config.InvalidConfigError so its own "run 'brief init
// --force'" fix is never lost to the generic one — carries
// writesFilesAnnotation the same way "new" and "finish" do. --json is
// registered but left out of the Use line itself (leaf help's own
// 80-column budget), the same as "check"'s and "new"'s own Use lines.
//
