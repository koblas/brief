// This file reaches the unexported run directly so a bound-agent role can
// be resolved against an injected home directory, the same reason
// init_internal_test.go does: through cli.Run a bare "developer" binding
// would also search the developer's own "~/.claude/agents", making these
// tests depend on the machine they happen to run on.
