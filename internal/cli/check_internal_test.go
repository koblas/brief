...
// that seam is unexported. check_test.go keeps its own symlink case plus
// every helper this file and check_hook_test.go/json_refusal_test.go, both
// out of this pass's scope, still call. check --hook itself stays entirely
// on disk (check_hook_test.go): runCheckHook's own FeatureContaining
// resolves through real os.Lstat/filepath.EvalSymlinks with no seam.
