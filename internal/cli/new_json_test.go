...
// .brief.yaml configuring state-file equal to specification-file makes
// the specification's write land first, then the state write's own
// O_CREATE|O_EXCL collide with it — the same seam
// internal/scaffold's Test_reports_a_state_write_that_cannot_be_committed_on_new_feature
// uses. files_changed must report true, not false, since the
// specification did land. The control arm reads that file back: it still
// carries the specification skeleton, proving files_changed's "true" is
// not vacuous.
func Test_new_feature_json_files_changed_is_true_when_the_state_write_fails(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(wd, ".brief.yaml"),
		[]byte("specification-file: SAME.md\nstate-file: SAME.md\n"), 0o600))
	...
	featureDir := filepath.Join(wd, "docs", "specifications", "payments")
	gotSpec, readErr := os.ReadFile(filepath.Join(featureDir, "SAME.md"))
