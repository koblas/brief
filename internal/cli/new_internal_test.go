// new's scenarios — every one, none of them OS-subject — moved onto
// rwfs.Mem here, reaching run() directly since withRootFS is unexported.
// new_step_test.go had no callers outside itself and is removed;
// new_json_test.go keeps only its own ENAMETOOLONG case, a real
